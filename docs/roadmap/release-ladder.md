...
flowchart TD
  ...
  R1 --> R2["R2 Restricted market-data contracts — current main"]
  R2 --> H3["Human R3 evidence review"]
  H3 -->|PASS and merge decision| R3["R3 Fixed-order deterministic simulation"]
  H3 -->|FAIL or BLOCKED| FIX["Bounded defect list; remain at R2"]
  R3 --> R4["R4 One-strategy research slice"]
...
```text
R2 main (stable)
  -> R3 human evidence review (NEXT)
  -> maintainer merge decision
  -> R4 StrategyProtocol + one baseline (separate future task)
```

Do not combine these arrows into one task. A failed or blocked R3 review stops
the sequence and produces a bounded defect list; it does not authorize R4.
