...
## Repository reality as of 2026-08-12
...
- The human R3 decision is `PASS` and is recorded in
  `docs/release/r3-promotion.md`. The R4 strategy slice is implemented only as
  a candidate and remains the sole `NEXT` promotion decision pending exact-head
  human review.
- Twelve Data and Coinbase connected qualification are both blocked/not opted
  in and are not release evidence.
- One synthetic trusted-local baseline candidate exists. No validated or
  promoted strategy, independent risk engine, account adapter, external
  order-submission path, or live capability exists.
...
| R4 — One-strategy research slice **NEXT / CANDIDATE** | R3 plus trusted `StrategyProtocol`, one transparent buy-and-hold baseline for one market, deterministic strategy-to-order-to-result path; no registry framework or optimization | R3 promoted/merged; synthetic BTC-USD fixture selected | Candidate evidence exists; exact-head contract/declared-data/unsupported-market/version-hash/determinism and synthetic-report review still requires human `PASS` | Remove baseline/adapter; fixed-order R3 remains usable | Research owner maintains one specification; separate approval to promote or add a second market |
...
flowchart TD
  ...
  R3 --> R4["R4 One-strategy research slice — candidate / NEXT review"]
...
R3 main (stable; human promotion PASS recorded)
  -> R4 StrategyProtocol + one baseline (candidate; exact-head review pending)
```

R3 promotion did not authorize R4 promotion automatically. The separately
approved implementation uses the exact synthetic BTC-USD fixture and now
requires its own exact-head evidence review before any R4 `CURRENT` claim.
